use crate::types::{
    AssessmentKey, Bet, BetKey, ClaimType, DataKey, Game, LastB, PrivateBet, ResultAssessment,
    ResultGame,
};
...
#[contractevent(topics = ["BettingGame", "Game_ResultbySupremeRemoved"], data_format = "vec")]
struct Game_ResultbySupremeRemoved {
    game_id: i128,
    result: BetKey,
}
#[contractevent(topics = ["BettingGame", "SupremeReward"], data_format = "vec")]
struct SupremeRewardEvent {
    game_id: i128,
    reward: i128,
}
...
#[contractevent(topics = ["BettingGame", "Setting_allUserHaveVoted"], data_format = "single-value")]
struct SettingAllUserHaveVotedEvent {
    setting: i128,
}
...
impl BettingEvents {
    ...
    pub fn user_honesty_points(e: &Env, user: Address, points: i128, position: u32) {
        UserHonestyPointsEvent {
            user,
            points,
            position,
        }
    ...
    pub fn all_voteSetting(e: &Env, setting: i128) {
        SettingAllUserHaveVotedEvent { setting }.publish(&e);
    }
    pub fn all_voteGame(e: &Env, game_id: i128) {
        GameAllUserHaveVotedEvent { game_id }.publish(&e);
    }
    pub fn game_resultbysupremeremoved(e: &Env, game_id: i128, result: BetKey) {
        Game_ResultbySupremeRemoved { game_id, result }.publish(&e);
    }
    pub fn supreme_reward(e: &Env, game_id: i128, reward: i128) {
        SupremeRewardEvent { game_id, reward }.publish(&e);
    }
}
